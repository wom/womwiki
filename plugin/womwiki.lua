...
if not vim.g.womwiki_disable_mappings then
	...
	vim.keymap.set("n", "<leader>wq", womwiki.capture, { desc = "womwiki quick capture" })
	vim.keymap.set("n", "<leader>wQ", womwiki.capture_with_location, { desc = "womwiki capture + location" })
	vim.keymap.set("v", "<leader>wq", womwiki.capture_visual, { desc = "womwiki capture selection + location" })
